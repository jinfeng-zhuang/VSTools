...
const unsigned long TriHidtv_crc32_table[256] = {
...
unsigned int Trid_Util_CRC32(unsigned long val, const void* ss, int len) {
    const unsigned char* s = ss;
    while (--len >= 0) val = TriHidtv_crc32_table[(val ^ *s++) & 0xff] ^ (val >> 8);
    return val;
}
