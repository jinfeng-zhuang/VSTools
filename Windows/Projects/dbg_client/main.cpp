...
int main(int argc, char* argv[])
{
    ...
    ret = dbg_init("10.86.79.93", 65528);
    if (0 == ret) {

        // Get Chip ID
        ret = dbg_host_read32(0xF500E000, &chip_id, 1);
        if (0 == ret) {
            vs_log(LOG_MASK_DBG, VS_LOG_INFO, "Chip ID: 0x%08x\n", chip_id);
        }

        if (chip_id != 0x9710) {
            vs_log(LOG_MASK_DBG, VS_LOG_WARNING, "Not a SX7B chip, not supported\n");
            goto END;
        }
        ...
        if (0 == ret) {
            vs_log(LOG_MASK_DBG, VS_LOG_INFO, "Current Frame Info Address: 0x%08x\n", cur_frame_info_addr);
        }

        if (0 == cur_frame_info_addr) {
            vs_log(LOG_MASK_DBG, VS_LOG_WARNING, "Current Frame Info Address is NULL\n");
            goto END;
        }
        ...
        if (0 == ret) {
            vs_log(LOG_MASK_DBG, VS_LOG_INFO, "Current Frame Info (%x): %dx%d%c @ %d, %s, Stride %d\n",
                frame_info.start_code,
        ...
        if (0 == ret) {
            vs_log(LOG_MASK_DBG, VS_LOG_INFO, "Current Luma Address: 0x%08x\n", cur_luma_addr);
        }
        ...
        if (0 == ret) {
            vs_log(LOG_MASK_DBG, VS_LOG_INFO, "Current Chroma Address: 0x%08x\n", cur_chroma_addr);
        }
        ...
        snprintf(filename, sizeof(filename), "%dx%d_%d.yuv", width, height, stride);

        luma_data = (unsigned char*)malloc(cur_luma_size);
        if (NULL == luma_data) {
            vs_log(LOG_MASK_DBG, VS_LOG_WARNING, "malloc luma failed\n");
            goto END;
        }
        ...
        if (NULL == chroma_data) {
            vs_log(LOG_MASK_DBG, VS_LOG_WARNING, "malloc chroma failed\n");
            goto END;
        }
