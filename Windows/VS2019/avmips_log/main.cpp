...
int main(int argc, char* argv[])
{
    ...
    if (argc != 3) {
        vs_log(LOG_MASK_AV, VS_LOG_WARNING, "Usage: %s <ip> <setting>\n", argv[0]);
        vs_log(LOG_MASK_AV, VS_LOG_WARNING, "Example: %s <ip> \"device:2, video_decode:1[fmvd]\"\n", argv[0]);
        return -1;
    }
    ...
    vs_log(LOG_MASK_AV, VS_LOG_INFO, "Current Log Address: 0x%08x\n", log_addr);
    ...
    vs_log(LOG_MASK_AV, VS_LOG_INFO, "Current Setting Address: 0x%08x\n", log_setting_addr);

    log_setting[strlen(log_setting)] = '\0';
    ret = dbg_host_write8(log_setting_addr, (unsigned char *)log_setting, strlen(log_setting) + 1);
    if (0 != ret) {
        goto END;
    }
    ...
    vs_log(LOG_MASK_AV, VS_LOG_INFO, "Parse Done\n");
    ...
    vs_log(LOG_MASK_AV, VS_LOG_INFO, "LOG DESC enable %d, level %d, total %d, rd %d, wr %d\n",
        local_desc.enable,
        local_desc.level,
        local_desc.total,
        local_desc.rd,
