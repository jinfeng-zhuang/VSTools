...
int main(int argc, char *argv[])
{
    ...
    if (g_sample_limit == 1) {
        es_desc_addr = get_addr(3 | 1<<6);
    }
    else if (g_sample_limit == 0) {
        es_desc_addr = get_addr(3);
    }
    else {
        return 0;
    }
    ...
    while (1) {
        ...
        group = (struct frameq_group *)g_es_descriptor;

        printf("rdy2DispQ %d\n", group->rdy2DispQ.wrIdx - group->rdy2DispQ.rdIdx);
